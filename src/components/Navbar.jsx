...
const Navbar = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const sections = ["Home", "Installation", "Features", "TechStack"];
      const navbarHeight = 120;
      const scrollPosition = window.scrollY + navbarHeight;
  ...
  return (
    <div>
      ...
      >
            ...
            {menuItems.map((item) => (
              <li key={item.id} className="relative group">
                ...
                >
                  {item.label}
                  <div
                    className={`absolute bottom-0 left-1/2 transform -translate-x-1/2 h-0.5 bg-[#2ee6f7] transition-all duration-300 ${
        ...
        <div
          className={`absolute top-full mt-4 left-1/2 transform -translate-x-1/2 w-4/5 transition-all duration-500 ${
            isOpen
          ...
          <div className="bg-[#0e0e0f] backdrop-blur-xl border-2 border-[#2ee6f7]/40 rounded-2xl shadow-[0_0_30px_#2ee6f7]/40 overflow-hidden">
            <ul className="relative flex flex-col items-center space-y-2 py-6 text-gray-300">
              ...
              <div className="flex space-x-6 mt-4 pt-4 border-t border-[#2ee6f7]/30">
                ...
                <a
                  href="https://www.linkedin.com/in/harshendra-m-2b8bb5299/"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="group relative p-4 rounded-full transition-all duration-300 hover:scale-110 hover:bg-[#2ee6f7]/20"
                >
